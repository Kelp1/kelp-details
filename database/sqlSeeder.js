...
fs.writeFile('menus1.json', '', () => console.log('cleared menus1'));
fs.writeFile('restaurants1.json', '', () => console.log('cleared restaurants1'));

Restaurants.collection.drop(); // only in seed1.js
Menus.collection.drop(); // only in seed1.js
...
function seeder() {
  ...
  for (let i = 0; i <= 2500000; i += 1) {
    ...
    for (let j = 0; j < Math.floor(Math.random() * 5) + 1; j += 1) {
      specCategories.push(categories[Math.floor(Math.random() * categories.length)]);
    }
    ...
    const restaurant = `
      ...
      ${Math.random() >= 0.5}|
      ${Math.random() >= 0.5}|
      ${attire[Math.floor(Math.random() * attire.length)]}|
      ${Math.random() >= 0.5}|
      ${Math.random() >= 0.5}|
      ${Math.random() >= 0.5}|
      ${alcohol[Math.floor(Math.random() * alcohol.length)]}|
      ${Math.random() >= 0.5}|
      ${wifi[Math.floor(Math.random() * wifi.length)]}|
      ${noiseLevel[Math.floor(Math.random() * noiseLevel.length)]}|
      ${Math.random() >= 0.5}|
      ${Math.random() >= 0.5}|
      ${Math.random() >= 0.5}|
      ${Math.random() >= 0.5}|
      ...
      ${specCategories}|
      ${hours[Math.floor(Math.random() * hours.length)]}-${hours[Math.floor(Math.random() * hours.length)]}|
      ${hours[Math.floor(Math.random() * hours.length)]}-${hours[Math.floor(Math.random() * hours.length)]}|
      ${hours[Math.floor(Math.random() * hours.length)]}-${hours[Math.floor(Math.random() * hours.length)]}|
      ${hours[Math.floor(Math.random() * hours.length)]}-${hours[Math.floor(Math.random() * hours.length)]}|
      ${hours[Math.floor(Math.random() * hours.length)]}-${hours[Math.floor(Math.random() * hours.length)]}|
    ...
    fs.appendFileSync('restaurants1.txt', `${JSON.stringify(restaurant)}\n`);
    ...
    const menu = `
      ...
      ${faker.lorem.word()}|
      ${Math.floor(Math.random() * 29) + 1 + 0.99}|
      ${[Math.floor(Math.random() * 10000000) + 1]}|
      ${[faker.image.food()]}
    `
    fs.appendFileSync('menus1.txt', `${JSON.stringify(menu)}\n`);

    if (i > 1 && i % 250000 === 0) {
      console.log(` completed write 1 batch ${batch} of 10`);
      batch += 1;
    }
  }
...
const insertRestaurants = 'mongoimport --db welp --collection restaurants --file restaurants1.json --numInsertionWorkers 2';
exec(insertRestaurants, (err) => {
  if (err) {
    console.log(err);
  }
  console.log('insert restaurants1 complete');
});

const insertMenus = 'mongoimport --db welp --collection menus --file menus1.json --numInsertionWorkers 2';
